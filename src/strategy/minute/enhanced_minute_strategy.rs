use crate::indicators::{
    ...
    oscillators::{calculate_rsi, calculate_williams_r, calculate_stochastic},
    trend::calculate_psar,
    volatility::{calculate_atr, calculate_bollinger_bands},
    volume::{calculate_obv, calculate_mfi, calculate_cmf},
};
use polars::prelude::*;
...
pub fn run_strategy(
    ...
    for i in max_window..df.height() {
        ...
        if ema_fast_val.is_nan() || ema_slow_val.is_nan() || rsi_val.is_nan() || 
           williams_r_val.is_nan() || stoch_k_val.is_nan() || stoch_d_val.is_nan() ||
           psar_val.is_nan() || atr_val.is_nan() || bb_upper_val.is_nan() || 
           bb_lower_val.is_nan() || mfi_val.is_nan() || cmf_val.is_nan() {
            buy_signals.push(0);
            sell_signals.push(0);
            stop_levels.push(0.0);
            target_levels.push(0.0);
        ...
        // Volume filter
        let volume_ok = !params.use_volume_filter || vol >= volume_sma_val * params.volume_threshold;
        
        // Time filter
        let time_ok = if params.use_time_filter && has_time_column {
            let time_str = match time_series.as_ref().unwrap().dtype() {
                DataType::String => time_series.as_ref().unwrap().str()?.get(i).unwrap_or("").to_string(),
            ...
            if parts.len() >= 2 {
                ...
                // Apply time filters
                let morning_ok = !params.use_time_filter || minutes_from_open >= params.filter_morning_minutes as i32;
                let lunch_ok = !params.filter_lunch_hour || hour < 12 || hour >= 13;
                let late_day_ok = !params.use_time_filter || minutes_to_close >= params.filter_late_day_minutes as i32;
        ...
        let williams_r_bullish = williams_r_val < -80.0 && williams_r_val > williams_r_vals.get(i-1).unwrap_or(-100.0);
        ...
        let williams_r_bearish = williams_r_val > -20.0 && williams_r_val < williams_r_vals.get(i-1).unwrap_or(0.0);
        ...
        // Count buy signals
        let mut buy_score = 0;
        if ema_cross_up { buy_score += 1; }
        if price_above_ema { buy_score += 1; }
        if rsi_oversold && rsi_rising { buy_score += 1; }
        if williams_r_bullish { buy_score += 1; }
        if stoch_cross_up || stoch_oversold { buy_score += 1; }
        if psar_bullish { buy_score += 1; }
        if price_at_bb_lower { buy_score += 1; }
        if mfi_oversold { buy_score += 1; }
        if cmf_positive { buy_score += 1; }
        if bullish_rsi_divergence { buy_score += 1; }
        
        // Count sell signals
        let mut sell_score = 0;
        if ema_cross_down { sell_score += 1; }
        if price_below_ema { sell_score += 1; }
        if rsi_overbought && rsi_falling { sell_score += 1; }
        if williams_r_bearish { sell_score += 1; }
        if stoch_cross_down || stoch_overbought { sell_score += 1; }
        if psar_bearish { sell_score += 1; }
        if price_at_bb_upper { sell_score += 1; }
        if mfi_overbought { sell_score += 1; }
        if cmf_negative { sell_score += 1; }
        ...
        let target_hit = in_position && high_val >= target_levels[i-1] && target_levels[i-1] > 0.0;
        
        // Generate final signals
        let buy_signal = if !in_position && buy_score >= params.min_buy_signals as i32 && volume_ok && time_ok {
            1
        } else {
            0
        };
        
        let sell_signal = if in_position && (sell_score >= params.min_sell_signals as i32 || stop_hit || target_hit) {
            1
        } else {
            0
        };
...
pub fn calculate_performance(
    ...
    for i in 0..close.len() {
        ...
        else if i < sell_signals.len() && shares > 0.0 && 
                (sell_signals[i] == 1 || 
