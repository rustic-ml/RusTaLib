use polars::prelude::*;
use crate::indicators::moving_averages::hull::calculate_hma;
...
fn test_hma_basic() {
    ...
    let window = 3;
    let result = calculate_hma(&df, "price", window).unwrap();
    let result_ca = result.f64().unwrap();
    // Check first two values are NaN
    for i in 0..(window-1) {
        assert!(result_ca.get(i).unwrap().is_nan());
    }
    // Check HMA values exist for remaining
    for i in (window-1)..df.height() {
        assert!(result_ca.get(i).is_some());
    }
}

#[test]
fn test_hma_window_one() {
    let df = create_test_df();
    let result = calculate_hma(&df, "price", 1).unwrap();
    let result_ca = result.f64().unwrap();
