...
mod tests {
    ...
    fn test_calculate_bollinger_bands_basic() {
        ...
        let (middle, upper, lower) =
            calculate_bollinger_bands(&df, window, num_std, "close").unwrap();
    ...
    fn test_bollinger_bands_edge_window() {
        ...
        for i in 0..df.height() {
            ...
            assert_eq!(
                mid_val,
                df.column("close").unwrap().f64().unwrap().get(i).unwrap()
            );
    ...
    fn test_bollinger_bands_accuracy_with_known_values() {
        ...
        assert!(
            upper_val >= 39.0 && upper_val <= 41.0,
            "Upper band should be close to 40.0, got {}",
            upper_val
        );
        assert!(
            lower_val >= -1.0 && lower_val <= 1.0,
            "Lower band should be close to 0.0, got {}",
            lower_val
        );
        ...
        let lower_val = lower.f64().unwrap().get(idx).unwrap();
        assert!(
            upper_val >= 49.0 && upper_val <= 51.0,
            "Upper band should be close to 50.0, got {}",
            upper_val
        );
        assert!(
            lower_val >= 9.0 && lower_val <= 11.0,
            "Lower band should be close to 10.0, got {}",
            lower_val
        );
    }
    ...
    fn test_bollinger_bands_with_different_std() {
        ...
        let (middle1, upper1, lower1) =
            calculate_bollinger_bands(&df, window, 1.0, "close").unwrap();

        // Test with 2 standard deviations
        let (middle2, upper2, lower2) =
            calculate_bollinger_bands(&df, window, 2.0, "close").unwrap();

        // Test with 3 standard deviations
        let (middle3, upper3, lower3) =
            calculate_bollinger_bands(&df, window, 3.0, "close").unwrap();

        // All middle bands should be identical
        assert_eq!(
            middle1.f64().unwrap().get(idx).unwrap(),
            middle2.f64().unwrap().get(idx).unwrap()
        );
        assert_eq!(
            middle2.f64().unwrap().get(idx).unwrap(),
            middle3.f64().unwrap().get(idx).unwrap()
        );

        // Bands should widen as std multiplier increases
        let band_width1 =
            upper1.f64().unwrap().get(idx).unwrap() - lower1.f64().unwrap().get(idx).unwrap();
        let band_width2 =
            upper2.f64().unwrap().get(idx).unwrap() - lower2.f64().unwrap().get(idx).unwrap();
        let band_width3 =
            upper3.f64().unwrap().get(idx).unwrap() - lower3.f64().unwrap().get(idx).unwrap();
