...
mod tests {
    ...
    fn test_gk_volatility_smoothing() {
        ...
        let spike_idx = 5;
        assert!(
            gk1.f64().unwrap().get(spike_idx).unwrap() > gk3.f64().unwrap().get(spike_idx).unwrap()
        );

        // The effect should linger longer with window=3
        let post_spike_idx = 7;
        assert!(
            gk3.f64().unwrap().get(post_spike_idx).unwrap()
                > gk1.f64().unwrap().get(post_spike_idx).unwrap()
        );
    }
}
