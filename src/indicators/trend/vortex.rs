...
pub fn calculate_vortex(df: &DataFrame, high_col: &str, low_col: &str, close_col: &str, period: usize) -> PolarsResult<(Series, Series)> {
    ...
    for i in 0..len {
        if i+1 >= period {
            let sum_tr: f64 = tr[(i+1-period)..=i].iter().filter(|x| !x.is_nan()).sum();
            let sum_vm_plus: f64 = vm_plus[(i+1-period)..=i].iter().filter(|x| !x.is_nan()).sum();
            let sum_vm_minus: f64 = vm_minus[(i+1-period)..=i].iter().filter(|x| !x.is_nan()).sum();
            if sum_tr != 0.0 {
