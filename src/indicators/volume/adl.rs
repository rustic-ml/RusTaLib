...
pub fn calculate_adl(df: &DataFrame, high_col: &str, low_col: &str, close_col: &str, volume_col: &str) -> PolarsResult<Series> {
    ...
    for i in 0..len {
        ...
        let mf_multiplier = if (high - low).abs() < std::f64::EPSILON {
            0.0
        } else {
            ((close - low) - (high - close)) / (high - low)
        };
        let mf_volume = mf_multiplier * volume;
        adl[i] = if i == 0 { mf_volume } else { adl[i-1] + mf_volume };
    }
