...
pub fn calculate_obv(df: &DataFrame) -> PolarsResult<Series> {
    ...
    // First value
    cumulative_obv = volume.get(0).unwrap_or(0.0);
    obv.push(cumulative_obv);
...
mod tests {
    ...
    fn test_obv_accuracy_with_precise_sequence() {
        ...
        let volume = Series::new("volume".into(), &[
            1000.0,
            2000.0,
            1500.0,
            3000.0,
        ...
        let expected_values = [1000.0, 3000.0, 4500.0, 1500.0, 1500.0, 4000.0, 2200.0, 2200.0, -1300.0, 3200.0];
    ...
    fn test_obv_trend_detection() {
        ...
        let up_volume = Series::new("volume".into(), &[1000.0, 1200.0, 1400.0, 1600.0, 1800.0, 2000.0]);
        ...
        let down_volume = Series::new("volume".into(), &[1000.0, 1200.0, 1400.0, 1600.0, 1800.0, 2000.0]);
        ...
        for i in 1..up_df.height() {
            assert!(up_obv.f64().unwrap().get(i).unwrap() > up_obv.f64().unwrap().get(i-1).unwrap());
        }
        
        // In a downtrend with increasing volume, OBV should consistently decrease
        for i in 1..down_df.height() {
            assert!(down_obv.f64().unwrap().get(i).unwrap() < down_obv.f64().unwrap().get(i-1).unwrap());
        }
        
        // The last value of the uptrend OBV should be the sum of all volumes
        let up_total = up_volume.f64().unwrap().sum().unwrap();
        assert_eq!(up_obv.f64().unwrap().get(up_df.height()-1).unwrap(), up_total);
        
        // The last value of the downtrend OBV should be the negative sum of all volumes (except the first)
        let down_total = -down_volume.f64().unwrap().sum().unwrap() + 2.0 * down_volume.f64().unwrap().get(0).unwrap();
        assert_eq!(down_obv.f64().unwrap().get(down_df.height()-1).unwrap(), down_total);
    }
} 