...
pub fn calculate_ultimate_oscillator(
    ...
    for i in 0..len {
        if i == 0 {
            ...
            bp[i] =
                close.get(i).unwrap_or(f64::NAN) - low.get(i).unwrap_or(f64::NAN).min(prev_close);
            tr[i] = high.get(i).unwrap_or(f64::NAN).max(prev_close)
                - low.get(i).unwrap_or(f64::NAN).min(prev_close);
        }
    }
    let mut uo = vec![f64::NAN; len];
    for (i, value) in uo.iter_mut().enumerate().take(len) {
        if i + 1 >= long {
            let sum = |v: &Vec<f64>, w: usize, idx: usize| {
                v[(idx + 1 - w)..=idx]
                    .iter()
                    .filter(|x| !x.is_nan())
                    .sum::<f64>()
            ...
            let m = avg(medium, i);
            let l = avg(long, i);
            if s.is_nan() || m.is_nan() || l.is_nan() {
                continue;
            }
            *value = 100.0 * (4.0 * s + 2.0 * m + l) / 7.0;
        }
    }
