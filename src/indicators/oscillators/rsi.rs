...
mod tests {
    ...
    fn test_calculate_rsi_basic() {
        ...
        for i in window-1..df.height() {
            if i < rsi.len() {  // Ensure we don't go out of bounds
                let val = rsi_ca.get(i);
        ...
        let len = up_rsi.len();
        if len > 3 {  // Check bounds before accessing
    ...
    fn test_calculate_rsi_edge_cases() {
        ...
        for i in 3..constant_df.height() {
            if i < len {  // Check bounds before accessing
                ...
                assert!(val.is_nan() || val > 50.0, 
                       "RSI for constant price should be undefined or high, got {}", val);
            }
        }
    }
    ...
    fn test_rsi_wilder_smoothing() {
        ...
        if idx < rsi.len() {
            ...
            if val.is_some() && !val.unwrap().is_nan() {
                ...
                assert!(rsi_val >= 25.0 && rsi_val <= 85.0,
                       "RSI should be in reasonable range for alternating gains/losses, got {}", rsi_val);
            }
        }
    }
    ...
    fn test_rsi_accuracy_with_precise_values() {
        ...
        if decline_idx < rsi.len() {
            ...
            if val.is_some() && !val.unwrap().is_nan() {
                ...
                assert!(rsi_val < 50.0, "RSI should be low after decline, got {}", rsi_val);
            }
        }
        ...
        if recovery_idx < rsi.len() {
            ...
            if val.is_some() && !val.unwrap().is_nan() {
                ...
                assert!(rsi_val > 35.0, "RSI should increase after recovery, got {}", rsi_val);
            }
        }
    }
    ...
    fn test_rsi_with_extreme_values() {
        ...
        if gain_idx < rsi.len() {
            ...
            if val.is_some() && !val.unwrap().is_nan() {
                ...
                assert!(rsi_val > 65.0, "RSI should be high after large increase, got {}", rsi_val);
            }
        }
        ...
        if loss_idx < rsi.len() {
            ...
            if val.is_some() && !val.unwrap().is_nan() {
                ...
                assert!(rsi_val < 60.0, "RSI should drop after large decrease, got {}", rsi_val);
            }
        }
    }
} 