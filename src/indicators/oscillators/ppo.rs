...
pub fn calculate_ppo(
    ...
    for i in 0..len {
        if i == 0 {
            ...
            ema_fast[i] = alpha_fast * close.get(i).unwrap_or(f64::NAN)
                + (1.0 - alpha_fast) * ema_fast[i - 1];
            ema_slow[i] = alpha_slow * close.get(i).unwrap_or(f64::NAN)
                + (1.0 - alpha_slow) * ema_slow[i - 1];
        }
    }
