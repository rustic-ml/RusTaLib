...
fn main() -> Result<(), PolarsError> {
    ...
    let opens = Series::new(
        "open".into(),
        &[
            100.0, 101.0, 102.0, 103.0, 102.5, 101.8, 102.3, 103.1, 104.2, 103.8,
            103.5, 104.1, 104.8, 105.2, 105.8, 106.3, 107.0, 106.5, 107.2, 107.8,
            108.3, 109.0, 109.5, 110.2, 110.8, 111.5, 112.0, 112.8, 113.5, 114.0,
            114.5, 115.0, 115.5, 116.0, 116.5, 117.0, 117.5, 118.0, 119.0, 121.0,
            124.0, 127.0, 130.0, 132.0, 131.0, 129.0, 127.0, 125.0, 123.0, 122.0,
            121.0, 120.0, 118.5, 117.0, 116.0, 115.5, 115.0, 114.5, 114.0, 113.5,
    ...
    let highs = Series::new(
        "high".into(),
        &[
            101.5, 102.5, 103.0, 103.5, 103.0, 102.5, 103.5, 104.5, 105.0, 104.5,
            104.0, 105.0, 105.5, 106.0, 106.5, 107.5, 108.0, 107.5, 108.0, 108.5,
            109.5, 110.0, 110.5, 111.0, 111.5, 112.5, 113.0, 113.5, 114.5, 115.0,
            115.5, 116.0, 116.5, 117.0, 117.5, 118.0, 118.5, 119.5, 121.0, 123.0,
            126.0, 129.0, 132.0, 133.0, 132.0, 130.0, 128.0, 126.0, 124.0, 123.0,
            122.0, 121.0, 119.5, 118.0, 117.0, 116.5, 116.0, 115.5, 115.0, 114.5,
    ...
    let lows = Series::new(
        "low".into(),
        &[
            99.5, 100.5, 101.0, 102.0, 101.5, 101.0, 102.0, 102.5, 103.0, 103.0,
            102.5, 103.0, 104.0, 104.5, 105.0, 106.0, 106.0, 106.0, 106.5, 107.0,
            108.0, 108.5, 109.0, 109.5, 110.0, 111.0, 111.5, 112.0, 113.0, 113.5,
            114.0, 114.5, 115.0, 115.5, 116.0, 116.5, 117.0, 117.5, 118.0, 120.0,
            123.0, 126.0, 129.0, 130.0, 128.0, 126.0, 124.0, 122.0, 121.0, 120.0,
            119.0, 118.0, 117.0, 116.0, 115.0, 114.5, 114.0, 113.5, 113.0, 112.5,
    ...
    let closes = Series::new(
        "close".into(),
        &[
            101.0, 102.0, 103.0, 102.5, 101.8, 102.3, 103.1, 104.2, 103.8, 103.5,
            104.1, 104.8, 105.2, 105.8, 106.3, 107.0, 106.5, 107.2, 107.8, 108.3,
            109.0, 109.5, 110.2, 110.8, 111.5, 112.0, 112.8, 113.5, 114.0, 114.5,
            115.0, 115.5, 116.0, 116.5, 117.0, 117.5, 118.0, 119.0, 121.0, 123.0,
            126.0, 129.0, 132.0, 131.0, 129.0, 127.0, 125.0, 123.0, 122.0, 121.0,
            120.0, 118.5, 117.0, 116.0, 115.5, 115.0, 114.5, 114.0, 113.5, 113.0,
    ...
    let volumes = Series::new(
        "volume".into(),
        &[
            150000, 155000, 160000, 158000, 152000, 148000, 155000, 165000, 160000, 152000,
            158000, 162000, 170000, 172000, 175000, 180000, 178000, 182000, 185000, 188000,
            190000, 195000, 198000, 200000, 205000, 210000, 215000, 220000, 225000, 230000,
            235000, 240000, 245000, 250000, 255000, 260000, 265000, 270000, 280000, 300000,
            350000, 400000, 450000, 500000, 400000, 350000, 300000, 275000, 250000, 225000,
            200000, 190000, 180000, 170000, 165000, 160000, 155000, 150000, 145000, 140000,
    ...
    println!("SMA length: {}, DataFrame height: {}", sma_20.len(), df.height());
    ...
    println!("RSI length: {}, DataFrame height: {}", rsi.len(), df.height());
    ...
    if rsi.len() < df.height() {
        ...
        println!("Padded RSI length: {}, DataFrame height: {}", padded_rsi.len(), df.height());
    ...
    println!("{}", result_df.select([
        "date", "close", "bb_middle", "bb_upper", "bb_lower", "rsi", "z_score", "signal"
    ])?);
    ...
    if signals_df.height() > 0 {
        ...
        println!("{}", signals_df.select([
            "date", "close", "z_score", "rsi", "signal"
        ])?);
    } else {
...
fn calculate_z_score(df: &mut DataFrame, window: usize) -> Result<(), PolarsError> {
    ...
    for i in 0..height {
        if i < window {
            ...
            let window_vec: Vec<f64> = window_slice.iter()
                .filter_map(|x| x)
                .collect();
            ...
            let variance: f64 = window_vec.iter()
                .map(|&x| (x - mean).powi(2))
                .sum::<f64>() / window_vec.len() as f64;
    ...
    println!("Z-score length: {}, DataFrame height: {}", z_scores.len(), height);
...
fn calculate_mean_reversion_signals(df: &mut DataFrame) -> Result<(), PolarsError> {
    ...
    println!("Signals length: {}, DataFrame height: {}", signals.len(), height);
