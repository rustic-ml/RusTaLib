...
fn main() -> Result<(), PolarsError> {
    ...
    let opens = Series::new(
        "open".into(),
        &[
            100.0, 101.5, 102.3, 103.8, 102.5, 
            101.2, 102.5, 103.3, 104.0, 103.1,
            102.8, 103.5, 104.2, 105.0, 104.5,
        ]
    );
    ...
    let highs = Series::new(
        "high".into(),
        &[
            101.8, 102.8, 103.5, 104.2, 103.1, 
            102.5, 103.8, 104.5, 105.2, 104.0,
            103.5, 104.2, 105.1, 106.0, 105.3,
        ]
    );
    ...
    let lows = Series::new(
        "low".into(),
        &[
            99.5, 101.0, 101.8, 102.9, 101.8, 
            100.5, 102.0, 102.8, 103.2, 102.5,
            102.1, 102.8, 103.8, 104.2, 103.8,
        ]
    );
    ...
    let closes = Series::new(
        "close".into(),
        &[
            101.5, 102.3, 103.2, 102.5, 101.8, 
            102.4, 103.2, 104.0, 103.5, 102.9,
            103.2, 104.2, 104.8, 104.5, 105.1, 
        ]
    );
    ...
    let volumes = Series::new(
        "volume".into(),
        &[
            150000.0, 180000.0, 160000.0, 140000.0, 130000.0,
            120000.0, 140000.0, 160000.0, 190000.0, 150000.0,
            140000.0, 160000.0, 180000.0, 210000.0, 190000.0,
        ]
    );
    ...
    let vwap_5_values: Vec<f64> = vwap_5.f64()?.iter().map(|opt_val| opt_val.unwrap_or(f64::NAN)).collect();
    ...
    // Display results
    println!("\nVWAP Analysis:");
    println!("{}", df_with_vwap.select([
        "date", "close", "volume", "vwap", "vwap_5day"
    ])?);
    ...
    println!("{}", final_df.select([
        "date", "close", "vwap", "price_vs_vwap"
    ])?);
